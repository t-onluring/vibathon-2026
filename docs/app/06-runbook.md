...
5. Jika selector publik `.tgme_widget_message_date time[datetime]` / `.tgme_channel_info_counter` tidak muncul, anggap topic tidak parseable dari public web HTML.
6. Jika tidak parseable, jangan publish placeholder topic; fallback ke parent group saja sampai ada bukti `topic_id` + HTML yang bisa di-parse.
7. Simpan catatan verifikasi di PR (topic, topic_id, timestamp cek).
